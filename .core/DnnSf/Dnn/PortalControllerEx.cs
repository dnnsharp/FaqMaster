...
namespace DnnSharp.FaqMaster.Core.DnnSf.Dnn
{
    static public class PortalControllerEx
    {
        ...
        public static string SanitizePortalAliasForQueryString(Uri currentUrl, PortalAliasInfo portalAlias)
        {
            ...
            return portalAlias.HTTPAlias.ToLower();
        }
        ...
        {
            return GetCurrentPortalAlias(uri.Host, uri.Port, uri.AbsolutePath);
        }

        public static PortalAliasInfo GetCurrentPortalAlias(string host)
        {
            int port = 80;
            if (host.IndexOf(':') != -1) {
                int.TryParse(host, out port);
            }

            return GetCurrentPortalAlias(host, port, "/");
        }

        public static PortalAliasInfo GetCurrentPortalAlias(string host, int port, string absolutePath)
        {
            
            // try to get the portal as it is
            var alias = GetAliasAndCheckAllForms(host, port, absolutePath);
            if (alias != null)
                return alias;

            // start from largest URL path and try to find an alias
            var aliases = PortalAliasController.GetPortalAliasLookup();
            var path = absolutePath.Substring(0, absolutePath.LastIndexOf('/')).TrimEnd('/');
            while (path != "") {

                alias = GetAliasAndCheckAllForms(host, port, path);
                if (alias != null)
                    return alias;
            ...
            return GetAliasAndCheckAllForms(host, port, path);
        }
        ...
        {
            var alias = GetCurrentPortalAliasInfo(portalId);
            return alias == null ? null : alias.HTTPAlias;
        }

        public static PortalAliasInfo GetCurrentPortalAliasInfo(int portalId)
        {
            ...
            if (HttpContext.Current == null)  // we can't determine from URL, there's no URL
                return (aliases[0] as PortalAliasInfo);
            ...
            var alias = GetAliasInfo(host, aliases);
            if (alias != null)
                return alias;
            ...
            if (host.IndexOf("www") == 0) {
                ...
                alias = GetAliasInfo(host, aliases);
                if (alias != null) {
                    alias.HTTPAlias = "www." + alias.HTTPAlias;
                    return alias;
                }
            }
        ...
        public static string MakeRelativeUrl(string absoluteUrl, int portalId)
        {
            ...
            return UriUtil.ToRelativeUrl(absoluteUrl);
        }
        ...
        static PortalAliasInfo GetAliasInfo(string host, IEnumerable aliases)
        {
            var request = HttpContext.Current.Request;
            foreach (PortalAliasInfo alias in aliases) {
                if (alias.HTTPAlias.IndexOf(host, StringComparison.OrdinalIgnoreCase) == 0) {
                    ...
                    if (request.RawUrl.IndexOf('/', 1) > 0) {
                        ...
                        if (firstPart.Equals("/DesktopModules", StringComparison.OrdinalIgnoreCase))
                            continue;
                        if (alias.HTTPAlias.IndexOf(host + firstPart, StringComparison.OrdinalIgnoreCase) == 0)
                            return alias;
                    }
                }
            }
