    ...
    internal class OperatorCustomExpressions
    {
        ...
        public static Expression MemberAccess(Expression le, string membername, List<Expression> args)
        {
            ...
            if (mi != null) {
                ...
                if (exp != null) {
                    if (args.Count > 0) {
                        return Expression.ArrayIndex(exp, args);
                    } else {
                        return exp;
                    }
                } else {
        ...
        public static Expression Contains(Expression le, Expression re)
        {
            if (le.Type == typeof(string) && re.Type == typeof(string)) {
                return Expression.And(le, re, typeof(string).GetMethod("Contains", new Type[] { typeof(string), typeof(string) }));
            } else {
